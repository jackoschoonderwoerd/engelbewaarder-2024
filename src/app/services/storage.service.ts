...
export class StorageService {
    ...
    uploadMultipleFiles(folderName, files: File[]) {
        ...
        const imageDownloadsUrlFilename: ImageDownloadUrlFilename[] = []
        return Promise.all(
            files.map(file => {
                return new Promise((resolve, reject) => {
                    this.upload(folderName, file).then((downloadUrl: string) => {
                        console.log(downloadUrl)
                        const downLoadUrlFilename: ImageDownloadUrlFilename = {
                            filename: file.name,
                            downloadUrl: downloadUrl
                        }
                        imageDownloadsUrlFilename.push(downLoadUrlFilename)
            ...
            .then((res: any) => {
                return imageDownloadsUrlFilename
            })
    }
